...
-- bibliographic data for a citation, that citation will *not* be looked up in
-- Zotero.
--
...
-- easier, and less error-prone way to fetch citations from Zotero. However,
-- you need to install the zotxt add-on to use it.
--
-- Web API
-- -------
--
...
-- listing the IDs of those groups in the metadata field
-- "zotero-public-groups"; accessing public groups does not require an API key.
--
--     pandoc -L pandoc-zotxt.lua -C <<EOF
--     ---
...
-- the first of each series of digits ("DoeTitle2020" translates to: "Doe",
-- "Title", "2020"). Easy Citekeys are split up at the first colon and the
-- last digit ("doe:2020title" translates to: "doe", "2020", "title").
...
-- by adding the citation key to the "extra" field of the item that the
-- citation key should match. Zotero's "extra" field is a list of CSL
-- key-value pairs; keys and values are separated by colons (":"), key-value
-- pairs by linefeeds. Use the key "Citation key" or "Citekey" to add a
-- citation key (e.g., "Citation key: DoeTitle2020"); case is insignifcant. If
-- you use Better BibTeX for Zotero, you can add the citation key it has
...
-- to the "references" metadata field. This speeds up subsequent processing,
-- because data that has already been fetched from Zotero need not be fetched
-- again.
--
...
-- if no input files were given, as relative to the current working directory.
-- The filename may contain environment variables, which should be enclosed in
-- `${...}` (e.g., `${HOME}` will be replaced with your home directory); any
-- series of *n* dollar signs will be replaced with *n* – 1 dollar signs,
-- so that you can escape dollar signs if they occur in filenames.
--
...
-- | `.yaml`    | CSL YAML   |
--
-- The items in the bibliography file are *not* updated or deleted
-- automatically. If you need to update them, delete the file, so that it will
-- be regenerated from scratch. You can also edit the file manually, of course.
--
-- For example:
--
--     pandoc -L pandoc-zotxt.lua -C <<EOF
...
--
-- Better BibTeX citation keys are sometimes, if rarely, misinterpreted as
-- Easy Citekeys, which may cause the wrong item to be fetched.
--
-- If this happens, prefix the citation key with its type and a colon (":"):
--
--     pandoc -L pandoc-zotxt.lua -C <<EOF
--     Force @betterbibtexkey:DoeTitle to be treated as BetterBibTeX citation
--     key.
--     EOF
--
...
-- assumed to be of one of the listed types only. See '[Citation key
-- types](#citation-key-types)' above.
--
-- * `zotero-connectors`: One or more Zotero ways to connect to Zotero:
--
--   | **Key**   | **Fetch data from**   |
--   | --------- | --------------------- |
...
--   could not be found, the Web API is searched next.
--
...
-- searched in addition to non-public groups. Groups should be public. Only
-- applies to the '[Web API](#web-api)'.
--
-- * `zotero-user-id`: A Zotero user ID. Looked up automatically if not given.
-- Only applies to the '[Web API](#web-api)'.
--
-- If a metadata field expects a list of values, giving a single-item list is
-- the same as giving a single item; the following two examples are equivalent:
--
--     pandoc -L pandoc-zotxt.lua -C <<EOF
--     ---
...
-- key or as an Easy Citekey. Set the 'zotero-citekey-types' metadata field or
-- prefix the citation key in question with its type and a colon (":") to
...
-- its metatable with the given metatables, in that order.
--
-- That is,
--
--    obj = Object(...)
...
-- is equivalent to
--
--    mt = update({}, getmetatable(Object), {__index = Object}, ...)
--    obj = setmetatable({}, mt)
--
...
Object.mt.__call = typed_args('table', '?table', '...')(
    function (proto, ...)
        return setmetatable(
            {},
            update({}, getmetatable(proto), {__index = proto}, ...)
        )
    end
)
...
--
-- `Object:new(...)` is short for `update(Object(), ...)`.
--
-- @tab ... Properties.
-- @treturn Object An object.
--
...
-- * Changes to the table are blocked.
--
-- @table environ
environ = {}
...
do
    ...
    --  of a path when run with Pandoc < v2.12.
    --
...
do
    ...
    -- @raise An error if the data cannot be represented in YAML.
    --
    -- @function yamlify
...
-- @raise An @{Error} if the opening and the closing tags are unbalanced.
...
do
    ...
    citekey.parsers.betterbibtexkey = typed_args('string')(
        function (ckey)
            ...
            return nil, 'not a Better BibTeX citation key.'
         end
    )
end
...
--- Check if a citation key is a Zotero ID.
--
-- @string ckey A citation key.
citekey.typifiers.key = typed_args('string')(
    function (ckey)
...
-- @treturn[2] nil `nil` if the given citation key is not of that type.
-- @treturn[2] string An error message.
citekey.matches_type = typed_args({
    typifiers = 'table'
}, 'string', 'string')(
    -- luacheck: ignore type
...
do
    ...
    citekey.guess_terms = typed_args({
...
do
    ...
    connectors.ZoteroWeb.query = typed_args('string', '?table')(
        function (ep, params)
            ...
            elseif not mt:match ';%s*charset="?utf%-?8"?%s*$' then
                return nil, 'Zotero response not encoded in UTF-8.'
            end
    ...
    connectors.ZoteroWeb.fetch = typed_args({
        ...
        function (self, ckey)
            -- luacheck: ignore err
            assert(ckey ~= '', 'citation key is the empty string.')
            if self.citekey_types:includes 'key' then
