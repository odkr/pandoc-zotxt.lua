...
.. code:: sh
   ...
   ...
   Look up @DoeTitle2020 via the Zotero Web API, too.
   EOF
...
zotero-api-key
   A Zotero API key.
   Only applies to the Zotero Web API. See '`Web API`_' above.
...
If a metadata field takes a list of values, but you only want to set a single
value, you can enter that value as a YAML scalar; for example, the following
...
Citation key matches wrong item
-------------------------------
...
Using groups with the Zotero Web API
------------------------------------
...
Zotero responds with "Request not allowed"
------------------------------------------
